...
void test_WindowLevel::isValid_ReturnsExpectedValues_data()
{
    ...
    QTest::newRow("invalid values - width == -0.0") << -0.0 << 12.0 << false;
    QTest::newRow("invalid values - width is nan") << std::numeric_limits<double>::quiet_NaN() << 5.0 << false;
    QTest::newRow("valid values - width < 1 (nearly 0)") << 1.5613731011E-14 << 12.0 << true;
    QTest::newRow("valid values - width < 1 (nearly 1)") << 0.99 << 12.0 << true;
