...
const uint PARTITIONS = 1;  // en quants trossos es parteix la imatge en cada dimensió (per reduir col·lisions)
const int VOLUME_MULTIPLIER = 1000000;  // multiplicador del volum a l'hora de convertir-lo en enter


// volum
cudaArray *gdVolumeArray;
...
cudaArray *gdTransferFunctionArray;
...
// mida del volum
uint gVolumeDataSize;
cudaExtent gVolumeDataDims;
float3 gVolumeDims;
...
__global__ void vomiKernel(float pv, float totalViewedVolume, cudaExtent volumeDataDims, float *vomi)
{
    ...
    float po = tex1Dfetch(gVoxelProbabilitiesTexture, i);
    float pov = tex1Dfetch(gViewedVolumesTexture, i) / totalViewedVolume;
    float pvo = pv * pov / po;

    if ( pvo > 0.0f ) vomi[i] += pvo * log2f( pvo / pv );
}
