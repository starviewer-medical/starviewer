...
template <class T>
double InformationTheory<T>::entropy( const QVector<T> &probabilities )
{
    ...
    if ( size >= MIN_SIZE_TO_USE_THREADS )
    {
        ...
        EntropyThread **threads = new EntropyThread*[nThreads];
        int sizePerThread = size / nThreads + 1;
        int start = 0, end = sizePerThread;

        for ( int i = 0; i < nThreads; i++ )
        {
            threads[i] = new EntropyThread( probabilities, start, end );
            threads[i]->start();
            start += sizePerThread;
            end += sizePerThread;
...
// H(X) = -sum[0,n)( p(x) log p(x) )
template <class T>
double InformationTheory<T>::entropy( const Histogram &histogram )
{
...
template <class T>
double InformationTheory<T>::kullbackLeiblerDivergence( const QVector<T> &probabilitiesP, const QVector<T> &probabilitiesQ )
{
    ...
    {
        int nThreads = QThread::idealThreadCount();
        KullbackLeiblerDivergenceThread **threads = new KullbackLeiblerDivergenceThread*[nThreads];
        int sizePerThread = size / nThreads + 1;
        int start = 0, end = sizePerThread;

        for ( int i = 0; i < nThreads; i++ )
        {
            threads[i] = new KullbackLeiblerDivergenceThread( probabilitiesP, probabilitiesQ, start, end );
            threads[i]->start();
            start += sizePerThread;
            end += sizePerThread;
            if ( end > size ) end = size;
...
template <class T>
double InformationTheory<T>::jensenShannonDivergence( double pi1, double pi2, const QVector<T> &probabilitiesP1, const QVector<T> &probabilitiesP2 )
{
    ...
    {
        int nThreads = QThread::idealThreadCount();
        ProbabilitiesMixThread **threads = new ProbabilitiesMixThread*[nThreads];
        int sizePerThread = size / nThreads + 1;
        int start = 0, end = sizePerThread;

        for ( int i = 0; i < nThreads; i++ )
        {
            threads[i] = new ProbabilitiesMixThread( pi1, pi2, probabilitiesP1, probabilitiesP2, probabilitiesMix, start, end );
            threads[i]->start();
            start += sizePerThread;
            end += sizePerThread;
            if ( end > size ) end = size;
...
template <class T>
class InformationTheory<T>::EntropyThread : public QThread {
    public:
...
template <class T>
class InformationTheory<T>::KullbackLeiblerDivergenceThread : public QThread {
    public:
...
template <class T>
class InformationTheory<T>::ProbabilitiesMixThread : public QThread {
    public:
