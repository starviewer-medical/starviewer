...
public:
    ...
    int size() const;
    /// Posa la mida de l'histograma a \p size.
    /// Si la nova mida és més gran que l'actual s'afegeixen noves entrades a 0 i si és més petita s'esborren les entrades sobrants.
    ...
    void add(int value);
    /// Decrementa el comptador d'ocurrències de \p value i el total.
    /// \p value ha de ser un valor en l'interval [0, size()) i el seu comptador d'ocurrències ha de ser > 0 (no es comprova).
