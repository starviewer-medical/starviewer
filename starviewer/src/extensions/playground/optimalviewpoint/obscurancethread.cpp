...
ObscuranceThread::ObscuranceThread( int id, int numberOfThreads, const QVector<Vector3> & directions, const TransferFunction & transferFunction, QObject * parent )
    : QThread(parent),
...
void ObscuranceThread::runDensity() // optimitzat
{
    ...
    for ( int i = m_id; i < nDirections; i += m_numberOfThreads )
    {
        ...
        int incX = m_increments[x], incY = m_increments[y], incZ = m_increments[z];
