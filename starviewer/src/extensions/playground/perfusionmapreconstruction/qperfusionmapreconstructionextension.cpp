...
void QPerfusionMapReconstructionExtension::changeMap( int value )
{
    ...
    case 0:     //CBV
        ...
        this->createColorMap(cbvImage, m_2DView);
        break;
    case 1:     //CBF
        m_2DView->setInput( m_map2Volume );
        m_2DView->resetView( Q2DViewer::Axial );
        m_2DView->removeAnnotation(Q2DViewer::NoAnnotation);
        m_2DView->resetWindowLevelToDefault();
        this->createColorMap(cbfImage, m_2DView);
        break;
    case 2:     //MTT
        m_2DView->setInput( m_map1Volume );
        m_2DView->resetView( Q2DViewer::Axial );
        m_2DView->removeAnnotation(Q2DViewer::NoAnnotation);
        m_2DView->resetWindowLevelToDefault();
        this->createColorMap(mttImage, m_2DView);
        break;
    default:
        break;
    }
...
void QPerfusionMapReconstructionExtension::computeDeltaR( )
{
    if(!m_DSCVolume)
    {
        QMessageBox::information(0,tr("Perfusion Map Extension"), tr("Some of the images have not been introduced") );
        return;
    }
...
void QPerfusionMapReconstructionExtension::computeMoments( )
{
    if(!m_DSCVolume)
    {
        QMessageBox::information(0,tr("Perfusion Map Extension"), tr("Some of the images have not been introduced") );
...
void QPerfusionMapReconstructionExtension::findAIF( )
{
...
/*    std::cout<<"init draw"<<std::endl;
    DrawerPoint* drawPoint = new DrawerPoint();
    double pos[3];
    pos[0]=index[0]*m_DSCVolume->getSpacing()[0] +m_DSCVolume->getOrigin()[0];
    pos[1]=index[1]*m_DSCVolume->getSpacing()[1] +m_DSCVolume->getOrigin()[1];
    pos[2]=index[2]*m_DSCVolume->getSpacing()[2] +m_DSCVolume->getOrigin()[2];
    drawPoint->setPosition(pos);
    std::cout<<"done setPosition"<<std::endl;

    m_2DView->getDrawer()->draw(drawPoint, QViewer::AxialPlane, index[2]);
  */
    ...
    {
        indexTemp[0] = t;
        aif[t] = deltaRImage->GetPixel(indexTemp);
        if(aif[t]>maxp) maxp = aif[t];
        if(aif[t]<minp) minp = aif[t];
    }
    for (t=0;t<tend;t++)
    {
        indexTemp[0] = t;
        p << QPointF(t*m_AIFView->size().width()/(tend),(deltaRImage->GetPixel(indexTemp)-minp)*m_AIFView->size().height()/(maxp-minp));
    }
...
void QPerfusionMapReconstructionExtension::computePerfusion( )
{
    QTime time;
    int time1 = 0;
    int time2 = 0;
    ...
    QVector<  double > timeseries(aif.size());
    QVector<  double > residueFunction(aif.size());
...
void QPerfusionMapReconstructionExtension::fftAIF( )
{
    fftaifreal = QVector<double>(aif.size());
    fftaifimag = QVector<double>(aif.size());
//     //Usant les fftw
...
//     free(out);
    ...
    size[0] = aif.size();  //les mostres temporals
    region.SetSize(size);
    region.SetIndex(start);
...
void QPerfusionMapReconstructionExtension::deconvolve( QVector<double> tissue, QVector<double>& residuefunc)
{
...
void QPerfusionMapReconstructionExtension::computeMomentsVoxel( QVector<double> v, double &m0, double &m1, double &m2)
{
    int i;
    m0=0.0;
    m1=0.0;
...
void QPerfusionMapReconstructionExtension::getOmega()
{
    //! returns omega axis for fft for dt=1
    omega = QVector<double>(aif.size());
    ...
    for(i=index; i<aif.size(); i++)
    {
        omega[i] = -(double)(aif.size()-i)/(double)(index-1)*omega_max;
    }
...
void QPerfusionMapReconstructionExtension::createColorMap(DoubleImageType::Pointer image, Q2DViewer* viewer)
{
    ...
    BoolIterator checkIter( checkImage, checkImage->GetBufferedRegion() );
    imIter.GoToBegin();
    checkIter.GoToBegin();
    int cont=0;
...
//         mapWriter2->Update();
        ...
        m_map2Volume = new Volume();
        m_map2Volume->setImages( m_DSCVolume->getPhaseVolume(0)->getImages() );
        try
        {
//           m_mapVolume->setData( outcaster->GetOutput() );
           m_map2Volume->setData( auxImage );
        }
        catch( itk::ExceptionObject & excep )
        {
            //std::cerr << "Exception caught !" << std::endl;
...
void QPerfusionMapReconstructionExtension::vsiEventHandler( unsigned long id )
{
    switch( id )
    {
...
//     emit showContextMenu( globalPoint );
    m_imageVSItype = QPerfusionMapReconstructionExtension::DSC;

    this->contextMenuEvent(new QContextMenuEvent(QContextMenuEvent::Mouse, point) );
}

void QPerfusionMapReconstructionExtension::contextMenuSEPreRelease()
{
    // Extret dels exemples de vtkEventQtSlotConnect
    ...
    //QPoint globalPoint = this->mapToGlobal( point );
//     emit showContextMenu( globalPoint );
    m_imageVSItype = QPerfusionMapReconstructionExtension::SEPre;

    this->contextMenuEvent(new QContextMenuEvent(QContextMenuEvent::Mouse, point) );
}

void QPerfusionMapReconstructionExtension::contextMenuSEPostRelease()
{
    // Extret dels exemples de vtkEventQtSlotConnect
    ...
    //QPoint globalPoint = this->mapToGlobal( point );
//     emit showContextMenu( globalPoint );
    m_imageVSItype = QPerfusionMapReconstructionExtension::SEPost;
