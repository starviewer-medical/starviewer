...
void ROITool::handleEvent( unsigned long eventID )
{
    switch( eventID )
    {
        case vtkCommand::LeftButtonPressEvent:
           if ( !m_2DViewer->getDrawer()->hasSelectedSet() ) //cas en que no hi ha cap conjunt de primitives seleccionat.
            {
                switch ( m_state )
                {
                    case STOPPED:
...
//             m_ellipseRepresentation->getText()->shadowOn();
            ...
            m_2DViewer->getDrawer()->drawEllipse( m_ellipseRepresentation->getEllipse(), m_2DViewer->getCurrentSlice(), m_2DViewer->getView() );
        break;
    }
...
void ROITool::stopROIAnnotation()
{
    ...
    m_2DViewer->getDrawer()->addSetOfPrimitives( m_ellipseRepresentation );
