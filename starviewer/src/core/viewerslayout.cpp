...
void ViewersLayout::restoreLayouts()
{
    ...
    setViewerSelected(m_vectorViewers.at(0));
    m_rows = 1;
    m_columns = 1;
}
...
void ViewersLayout::removeColumns( int columns )
{
    ...
    while( columns > 0 && m_columns > 1 )
    {
        ...
        while (rows < m_viewersLayout->rowCount() )
        {
            oldViewer = m_vectorViewers.at(posViewer);
...
void ViewersLayout::showRows( int rows )
{
    ...
    while( rows > 0 )
    {
        ...
        {
            viewer = m_vectorViewers.at((m_totalColumns * m_rows) + numColumn);
            viewer->show();
        }
        m_rows++;
        rows--;
...
void ViewersLayout::showColumns( int columns )
{
    ...
    while( columns > 0 )
    {
        ...
        {
            viewer = m_vectorViewers.at((m_totalColumns * numRow) + m_columns);
            viewer->show();
        }
        m_columns++;
        columns--;
