...
OldVoxelInformationTool::OldVoxelInformationTool( Q2DViewer *viewer, QObject *parent )
 : /*Tool(parent),*/ m_isEnabled(false), m_voxelInformationCaption(0)
{
    m_2DViewer = viewer;
    createCaptionActor();

    connect( m_2DViewer, SIGNAL( sliceChanged(int) ), SLOT( isNeededUpdateVoxelInformation() ) );
}

OldVoxelInformationTool::~OldVoxelInformationTool()
{
}

void OldVoxelInformationTool::handleEvent( unsigned long eventID )
{
    switch( eventID )
    {
...
void OldVoxelInformationTool::enable( bool enable )
{
    m_isEnabled = enable;
    if(!enable)
        m_voxelInformationCaption->VisibilityOff();
...
void OldVoxelInformationTool::createCaptionActor()
{
    double xyz[3];
...
void OldVoxelInformationTool::updateVoxelInformation()
{
    double xyz[3];
...
void OldVoxelInformationTool::depthAccordingViewAndSlice( double xyz[3] )
{
...
void OldVoxelInformationTool::placeText( double textPosition[3] )
{
...
int* OldVoxelInformationTool::viewportDimensions()
{
    return m_2DViewer->getRenderer()->GetSize();
}

bool OldVoxelInformationTool::captionExceedsViewportTopLimit()
{
...
bool OldVoxelInformationTool::captionExceedsViewportRightLimit()
{
    int *dimensions = viewportDimensions();
...
bool OldVoxelInformationTool::captionExceedsViewportLimits()
{
    return ( captionExceedsViewportTopLimit() || captionExceedsViewportRightLimit() );
}

void OldVoxelInformationTool::correctPositionOfCaption( int correctPositionInViewPort[2] )
{
...
void OldVoxelInformationTool::isNeededUpdateVoxelInformation()
{
    if ( m_isEnabled )
    {
        updateVoxelInformation();
