...
void ShutterFilter::setView(int view)
{
    m_view = view;
}

void ShutterFilter::setReverseStencilOn()
{
    m_reverseStencil = true;
}

void ShutterFilter::setReverseStencilOff()
{
    m_reverseStencil = false;
}

void ShutterFilter::setBackgroundTransparent()
{
    m_background = m_inputData->getVtkData()->GetScalarRange()[0] - 1; // Signe pel transparent a la LUT de Window Level
    DEBUG_LOG(QString("Background value: %1").arg(m_background));
}
                ...
                if (presentationStateHandler->haveShutter(DVPSU_bitmap))
                {
                    ...
                    for (int overlayIndex = 0; overlayIndex < overlays; overlayIndex++)
                    {
                        if (presentationStateHandler->overlayIsSuitableAsBitmapShutter(overlayIndex))
                        {
                            ...
                            status = presentationStateHandler->getOverlayData(layer, overlayIndex, data, width, height, left, top, isROI, foreground);
...
{
    DEBUG_LOG(QString("Coordenades shutter RECTANGULAR: [LV,RV,UH,LH] %1,%2,%3,%4").arg(leftVertical).arg(rightVertical).arg(upperHorizontal).arg(lowerHorizontal));
    double origin[3];
    m_inputData->getOrigin(origin);
...
void ShutterFilter::setPolygonalShutter(QList<double *> vertexs)
{
    int xCoordinate = -1, yCoordinate = -1, zCoordinate = -1;
    ...
    DEBUG_LOG(QString("POLYGONAL shutter, number of points: %1").arg(vertexs.size()));
    ...
    {
        points->InsertPoint(i, vertexs.at(i)[xCoordinate], vertexs.at(i)[yCoordinate], origin[zCoordinate] - 1);
    }

    vtkCellArray *vertexsCell = vtkCellArray::New();
    vertexsCell->InsertNextCell(vertexs.size() + 1);
    for (int i = 0; i < vertexs.size(); i++)
    {
        vertexsCell->InsertCellPoint(i); // 0,1,2,3,n
    }
    vertexsCell->InsertCellPoint(0);

    vtkPolyData *polyData = vtkPolyData::New();
    polyData->SetPoints(points);
    polyData->SetLines(vertexsCell);
...
vtkImageData *ShutterFilter::getOutput()
{
    ...
    if (m_inputData)
    {
        ...
        if (m_polygonalPolyDataStencil)
        {
            ...
            if (m_reverseStencil)
            {
                polygonalStencil->ReverseStencilOn();
            }
            else
            {
        ...
        if (m_circularPolyDataStencil)
        {
            ...
            else if (rectangularStencil) // Sinó mira si n'hi de rectangular per concatenar
            {
