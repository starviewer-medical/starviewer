...
mkdir hangingprotocols/Specific
cp -r $STARVIEWER_DEPLOYMENT_DIR_BASE/hangingprotocols/IDI/* hangingprotocols/Specific
...
# TODO Generate pdfs from fodts (copy cesymbols from starviewer-deployment before)
cp $STARVIEWER_SOURCE_DIR_BASE/doc/help/usermanual/*.pdf .
cp $STARVIEWER_SOURCE_DIR_BASE/doc/help/shortcuts/*.pdf .
cp $STARVIEWER_SOURCE_DIR_BASE/doc/help/quickstartguide/*.pdf .
# TODO Maybe this chmod can be omitted when the pdfs are autogenerated
chmod 664 *.pdf
...
# Qt translations
# ---------------
mkdir $STARVIEWER_INSTALL_DIR_BASE/translations
pushd $STARVIEWER_INSTALL_DIR_BASE/translations

cp $QTTRANSLATIONSDIR/qt_ca.qm .
cp $QTTRANSLATIONSDIR/qtbase_ca.qm .
cp $QTTRANSLATIONSDIR/qtmultimedia_ca.qm .
cp $QTTRANSLATIONSDIR/qtscript_ca.qm .
cp $QTTRANSLATIONSDIR/qtxmlpatterns_ca.qm .

cp $QTTRANSLATIONSDIR/qt_es.qm .
cp $QTTRANSLATIONSDIR/qtbase_es.qm .
cp $QTTRANSLATIONSDIR/qtmultimedia_es.qm .
cp $QTTRANSLATIONSDIR/qtscript_es.qm .
cp $QTTRANSLATIONSDIR/qtxmlpatterns_es.qm .
